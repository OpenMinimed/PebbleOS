...
- **The dict is per build.** Log lines are stored hashed and the hashes change every build, so an
  ...
  `.pbz` as `sake-spike-<hash>-<desc>.loghash.json` (`<hash>` = the build's short git commit hash);
  pass it with `--dict`. Without the right dict the lines come back as raw `NL:xxxx`.
